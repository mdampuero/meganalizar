...
$.fn.show_tooltip = function(text, color, validate_position) {
    $(this).tooltip('destroy');
    var rid = 'tlt_' + parseInt(new Date().getTime());
    var marker = '<div id="' + rid + '"</div>';
    $(this).tooltip(
            {title: text + marker, html: true, trigger: 'manual', placement: validate_position});
    $(this).tooltip('show');
    $('#' + rid).parent().css({'background-color': color});
    $('#' + rid).parent().prev().css({'border-top-color': color});
    if(validate_position == 'right'){
        $('#' + rid).parent().prev().css({'border-top-color': 'transparent'});
...
$.fn.show_err_tooltip = function(text, validate_position) {
    $(this).show_tooltip(text, 'rgba(255,0,0,0.8)', validate_position)
}
...
$.fn.bt_validate = function() {
    ...
    $.bt_validate.form.find('input[data-validate],select[data-validate],textarea[data-validate]').blur(function() {
        ...
        var validate_position = $(this).attr('data-validate-position');
        ...
        for (var i in validate_params) {
            ...
            if (typeof(res) != 'string') {
                if (!res) {
                    ...
                    $(this).show_err_tooltip(tl_text, validate_position);
