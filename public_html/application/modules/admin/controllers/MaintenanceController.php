...
class Admin_MaintenanceController extends Zend_Controller_Action {
    ...
    protected $IDRecurso;
    var $fields = array(
     array('field' => 'borrar_registros_logs', 'label' => 'Borrar todos los registros de Logs'),
     array('field' => 'test_web_service', 'label' => 'Probar Webservice')
     );
    ...
    public function indexAction() {
        try {
            ...
            if ($this->getRequest()->isPost()){
                ...
                $message = "";
                if ($_POST["borrar_registros_logs"] == true){
                    $message.="<p>Se vació la tabla '" . $this->log->clearData() . "'</p>";
                }
                if ($_POST["test_web_service"] == true){
                    echo '<pre>';
                    print_r($_POST);
                    echo '</pre>';
                    exit();
                    $message.="<p>Se vació la tabla </p>";
                }
